...
CREATE PROCEDURE insertar_factura 
	...
	@efectivo BIT, @moneda BIT, @monto MONEY, @total MONEY, @institucion_cedida VARCHAR(100), @banco VARCHAR(100),
...
BEGIN
	...
	@moneda, @monto, @total, @institucion_cedida, @banco, @observaciones, @cod_contratacion)
END
...
AS BEGIN
	...
	transferencia = @transferencia, estado = @estado, efectivo = @efectivo, moneda = @moneda, monto = @monto, total = @total,
...
AS BEGIN
	...
	transferencia AS 'Transferencia', estado AS 'Estado', efectivo AS 'Efectivo', moneda AS 'Moneda', total AS 'Total', 
